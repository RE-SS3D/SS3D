...
**Claude Design, prompt 1 — AI's camera/control view:**
> Build the AI's own screen: a real through-the-lens camera feed with a quick-switch camera list, and an Area control panel (door/light/APC toggles) tied to whatever Area is currently in view. This isn't main HUD's minimal-chrome treatment and isn't lobby's dense panel treatment either — find the middle ground described in §8.

**Claude Design, prompt 2 — upload console and audit trail:**
> Build the upload console's diegetic device screen: a physical law-module insertion action, the current ordered law list, and a scrollable audit log showing law changes and law-check queries, timestamped. Include one log entry visually flagged as originating outside the normal access pattern.

**Claude Design, prompt 3 — cyborg module radial and damage:**
> Show a cyborg's module tool-select radial (reusing the existing radial menu visual language) and the character model showing EMP/physical damage — sparking, scorching, exposed wiring — as the doll-replacement equivalent of a wound.

**Cursor, prompt 1 — data contract first:**
> Here's the AI & cyborgs design doc. Define the AI core record (Area/power binding, backup window, integrity), the cyborg chassis record (module type, brute/burn/EMP pools, battery), the law record (ordered list, audit log entries), and the action-interface definition from §9 (camera request, door/light/APC toggle, comms message, borg order) as a driver-agnostic shape. Show me all four before wiring any UI.

**Cursor, prompt 2 — one vertical slice, human-driven only:**
> Implement AI camera viewing and door/light/APC control end to end, wired to real Area data and a human player as the sole driver of the action interface. Laws, subversion, and cyborgs come after this is reviewed.

**Cursor, prompt 3 — subversion (after the above is reviewed):**
> Wire the AI core and upload console into the existing hacking-interface device model: crew/engineer/hacker tiered views, the bypass path for an unauthorized law insert, and the audit log capturing it. Reuse the FDU's existing panel logic rather than duplicating it.
