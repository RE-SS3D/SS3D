...
**Claude Design, prompt 1 — helm console:**
> Build the helm console's diegetic device screen, reusing the FDU/fabricator visual language: a real proximity/radar display showing nearby objects as actual relative positions and velocity vectors (not abstract blips), a thrust/heading readout, and a docking-alignment guide that only appears within range of a port, showing current alignment against the required tolerance.

**Claude Design, prompt 2 — autopilot vs. manual handoff and a breach:**
> Show a shuttle flying an automated route (waypoint line, steady approach into a dock). Then show a player taking manual control at the helm — same shuttle, now with direct thrust/heading input — followed by a collision into a station structure: the struck Area's integrity dropping, a breach effect at that section, and a hull-breach alert chip lighting up in the HUD per the existing alert-stack pattern.

**Cursor, prompt 1 — data contract first:**
> Here's the shuttles design doc. Define the shuttle record (root transform/rigidbody, its own Area list with per-Area integrity, docking ports with alignment tolerance, power source feeding its own APC, control state: autopilot-with-route vs. manual-occupied) and the docking resolution function (alignment check → clamp timed action → Area-boundary connection, per §2–3). Show me both before wiring any movement behavior.

**Cursor, prompt 2 — one vertical slice:**
> Implement one shuttle on a fixed autopilot route between two docks end to end: real movement, obstacle-holding behavior, alignment-gated docking/undocking, and its own Area-based power drawing from its own APC. Manual piloting, collision/breach resolution, and shore power at dock come after this is reviewed.

**Cursor, prompt 3 — manual control and collision (after the above is reviewed):**
> Add manual helm control to the same shuttle: direct thrust/heading input, autopilot handoff in both directions, and the collision-resolution function from §6 (impact force → per-Area integrity check → breach, tied into atmospherics and the per-limb damage model for crew in the affected Area).
