...
namespace SS3D.Content.Systems.Player.IK
{
    public class SimpleBodyPartLookAt : MonoBehaviour
    {
        [SerializeField] public Camera bodyPartCamera;
        ...
        private void Start()
        {
            ...
            bodyPartCamera = CameraManager.singleton.playerCamera.GetComponent<Camera>();
        }
        ...
        private Vector3 GetMousePosition(bool changeYAxis)
        {
            Ray ray = bodyPartCamera.ScreenPointToRay(Input.mousePosition);
            Vector3 mousePosition = ray.origin - ray.direction * (ray.origin.y / ray.direction.y);
            mousePosition = new Vector3(mousePosition.x, changeYAxis ? mousePosition.y : transform.position.y, mousePosition.z);
