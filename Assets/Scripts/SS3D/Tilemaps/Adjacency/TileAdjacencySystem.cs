    ...
    public class TileAdjacencySystem : NetworkedSystem
    {
        ...
        private void Update()
        {
            ...
            if (Input.GetKeyDown(KeyCode.Space))
            {
                ...
                _tileSystem.GetTile(pos).Objects.TryGetValue(TileLayer.Turf, out TileObject tileObject);

                TileLayer layer = tileObject.Layer;
                TileObjects type = tileObject.Id;

                if (IsTileType(new Vector3Int(pos.x + 1, pos.y, pos.z), type, layer))
                {
                ...
                if (IsTileType(new Vector3Int(pos.x, pos.y, pos.z + 1), type, layer))
                {
                ...
                if (IsTileType(new Vector3Int(pos.x + 1, pos.y, pos.z + 1), type, layer)) {
        ...
        private bool IsTileType(Vector3Int pos, TileObjects type, TileLayer layer)
        {
            _tileSystem.GetTile(pos).Objects.TryGetValue(layer, out TileObject tile);
            if (tile == null)
                return false;
