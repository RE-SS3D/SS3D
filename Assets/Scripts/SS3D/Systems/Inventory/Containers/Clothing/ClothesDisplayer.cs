...
namespace SS3D.Systems.Inventory.Containers
{
    ...
    public class ClothesDisplayer : NetworkActor
    {
        ...
        private void ClothedBodyPartsOnChange(SyncListOperation op, int index, ClothDisplayData oldData, ClothDisplayData newData, bool asServer)
        {
            ...
            switch (op)
            {
                ...
                case SyncListOperation.Add:
                {
                    ...
                    Mesh newClotheMesh = newItem.gameObject.GetComponent<MeshFilter>().sharedMesh;
                    renderer.sharedMesh = newClotheMesh;
                    break;
                }
        ...
        private void HandleContainerContentChanged(AttachedContainer container, Item oldItem, Item newItem, ContainerChangeType type)
        {
            ...
            if (container.GetComponent<InventoryClothContainer>() == null)
            {
                return;
            }
        ...
        private void AddCloth(Item item)
        {
            ...
            if (bodypart.TryGetComponent(out NetworkObject networkedBodyPart))
            {
