...
namespace SS3D.Systems.Tile
{
    ...
    public class TileSystem : NetworkSystem
    {
        ...
        public void LoadAll(bool softLoad)
        {
            ...
            foreach (MapSaveObject mapSave in saveMapObject.saveObjectList)
            {
                TileMap map = null;
                if (softLoad)
                {
                    ...
                    if (!found)
                    {
                        Debug.LogWarning("Map was not found when reinitializing: " + mapSave.mapName);
                        continue;
                    }

                    map.Setup(mapSave.mapName);
                    map.Load(mapSave, true);
                    Debug.Log("Tilemaps soft loaded");
                }
                else
                {
                    map = AddTileMap(mapSave.mapName);
                    map.Load(mapSave, false);
