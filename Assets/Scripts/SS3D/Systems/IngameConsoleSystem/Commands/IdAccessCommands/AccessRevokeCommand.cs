...
{
    public class AccessRevokeCommand : Command
    {
        public override string ShortDescription => "Revoke an access level from on-person ID record";
        public override string LongDescription =>
            "Removes one access level from the crew record bound to the player's currently resolved ID card.";
        ...
        public override string Perform(string[] args, NetworkConnection conn = null)
        {
            ...
            AccessMask updated = record.Access.Without(values.Level);
            if (!IdAccessCommandUtilities.TrySetRecordAccess(inventory, updated, out string setError))
            {
                return setError;
            }

            return $"Revoked {values.Level}. Record access is now: {IdAccessCommandUtilities.FormatAccessMask(updated)}";
        }
