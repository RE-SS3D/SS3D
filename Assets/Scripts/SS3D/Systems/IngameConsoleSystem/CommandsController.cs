...
namespace SS3D.Systems.IngameConsoleSystem
{
    public class CommandsController : NetworkActor
    {
        ...
        [SerializeField]
        private ConsolePanelView _console;
        ...
        {
            IEnumerable<Type> commands = Assembly.GetAssembly(typeof(Command)).GetTypes().Where(iType => iType.IsClass && iType.IsSubclassOf(typeof(Command)) && !iType.IsAbstract);
