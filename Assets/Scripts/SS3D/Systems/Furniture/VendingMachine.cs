...
namespace SS3D.Systems.Furniture
{
    ...
    public class VendingMachine : InteractionSource, IInteractionTarget
    {
        ...
        public void DispenseProduct(int productIndex)
        {
            ...
            {
                Subsystems.Get<AudioSubSystem>().PlayAudioSource(Audio.AudioType.Sfx, Sounds.BikeHorn, Position, NetworkObject, false, 0.7f, 1, 1, 3);
                return;
            }

            _powerConsumer.UseMachineOnce();
            productToDispenseStock.Stock--;
            Subsystems.Get<AudioSubSystem>().PlayAudioSource(Audio.AudioType.Sfx, Sounds.Can1, Position, NetworkObject, false, 0.7f, 1, 1, 3);
            ...
            itemSubSystem.SpawnItem(productToDispenseStock.Product.name, _dispensingTransform.position, quaternion);
        }
