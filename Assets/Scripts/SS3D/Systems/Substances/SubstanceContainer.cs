...
namespace SS3D.Substances
{
    ...
    {
        public delegate void ContentChanged(SubstanceContainer container);
        ...
        /// </summary>
        private readonly List<SubstanceEntry> _substances = new();

        /// <summary>
        /// A list of initial substances in this container
        /// </summary>
        [FormerlySerializedAs("InitialSubstances")]
        [SerializeField]
        private List<SubstanceEntry> _initialSubstances;
        ...
        public static void ProcessContainer(SubstanceContainer container)
        {
            ...
            foreach (Recipe recipe in registry.Recipes)
            {
                ...
                for (int i = 0; i < moles.Length; i++)
                {
                    Recipe.RecipeComponent component = recipe.Ingredients[i];
                    Substance substance = registry.FromType(component.Type);
                ...
                {
                    Recipe.RecipeComponent component = recipe.Results[i];
                    Substance substance = registry.FromType(component.Type);
                ...
                // Add results
                foreach (Recipe.RecipeComponent component in recipe.Results)
                {
        ...
        {
            SubstanceEntry entry = Substances.FirstOrDefault(x => x.Substance == substance);
            return entry.Substance == null ? 0f : entry.Substance.MillilitersPerMilliMoles * entry.MilliMoles;
        }

        protected override void OnStart()
        {
            foreach (SubstanceEntry substance in _initialSubstances)
            {
