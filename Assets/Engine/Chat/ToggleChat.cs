...
namespace SS3D.Engine.Inventory.UI
{
	...
	public class ToggleChat : MonoBehaviour
	{
        ...
        private void Update()
        {
			...
			if (InputHelper.inputs.ui.toggleChatDown.ReadValue<bool>() && EventSystem.current.currentSelectedGameObject == null)
			{
				Toggle();
			}
			// Button to focus chat. Also unhides chat if pressed.
			if (InputHelper.inputs.ui.focusChatDown.ReadValue<bool>() && EventSystem.current.currentSelectedGameObject == null)
			{
				if (!isShowing)
				{
					Toggle();
