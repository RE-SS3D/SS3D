...
/// <summary>
/// Custom Logger with categories support.
/// </summary>
public static class Logger
{
    private static LoggerPreferences _loggerPreferences;
    ...
    public static void RefreshPreferences()
    {
        ...
        _loggerPreferences = JsonUtility.FromJson<LoggerPreferences>(File.ReadAllText(Path.Combine(path, "custom.json")));

        LogOverrides.Clear();

        foreach (LogOverridePref pref in _loggerPreferences.logOverrides)
        {
    ...
    public static void SetLogLevel(Category _category, LogLevel level)
    {
        ...
        var index = _loggerPreferences.logOverrides.FindIndex(x => x.category == _category);
        if (index != -1)
        {
            _loggerPreferences.logOverrides[index].logLevel = level;
        }
        else
        {
            _loggerPreferences.logOverrides.Add(new LogOverridePref() { category = _category, logLevel = level });
        }
    ...
    public static void SaveLogOverrides()
    {
        var path = Path.Combine(Application.streamingAssetsPath,
            "LogLevelDefaults/");
        File.WriteAllText(Path.Combine(path, "custom.json"), JsonUtility.ToJson(_loggerPreferences));
    }
